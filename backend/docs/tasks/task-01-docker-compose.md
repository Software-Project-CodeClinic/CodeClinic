...
| 서비스 | 이미지 | 포트 | 비고 |
|--------|--------|------|------|
| `timescaledb` | `timescale/timescaledb:latest-pg17` | `5432:5432` | 공격 로그 저장소 |
...
- `timescale/timescaledb:latest-pg17` 이미지는 `pg_isready` health check 지원 → `depends_on` 활용 시 `condition: service_healthy` 사용
